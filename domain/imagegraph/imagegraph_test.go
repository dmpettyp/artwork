...
func TestNode_SetConfig(t *testing.T) {
	...
	t.Run("accepts valid JSON array", func(t *testing.T) {
		ig, _ := imagegraph.NewImageGraph(imagegraph.MustNewImageGraphID(), "test")
		nodeID := imagegraph.MustNewNodeID()
		ig.AddNode(nodeID, imagegraph.NodeTypeInput, "node", "")
	...
	t.Run("accepts valid JSON primitives", func(t *testing.T) {
		ig, _ := imagegraph.NewImageGraph(imagegraph.MustNewImageGraphID(), "test")
		nodeID := imagegraph.MustNewNodeID()
		ig.AddNode(nodeID, imagegraph.NodeTypeInput, "node", "")
		...
		for _, config := range testCases {
			err := node.SetConfig(config)
			if err != nil {
				t.Errorf("expected no error for config %q, got %v", config, err)
			}
		}
	})
	...
	t.Run("accepts empty string", func(t *testing.T) {
		...
		if node.Config != "" {
			t.Errorf("expected empty config, got %q", node.Config)
		}
	})
	...
	t.Run("does not emit event for empty config", func(t *testing.T) {
		ig, _ := imagegraph.NewImageGraph(imagegraph.MustNewImageGraphID(), "test")
		nodeID := imagegraph.MustNewNodeID()
		ig.AddNode(nodeID, imagegraph.NodeTypeInput, "node", "")
		ig.ResetEvents()
		...
		events := ig.GetEvents()
		if len(events) != 0 {
			t.Fatalf("expected 0 events for empty config, got %d", len(events))
		}
	})
}
